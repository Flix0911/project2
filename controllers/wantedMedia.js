...
router.get("/", async (req, res) => {
    //find albums that are wanted
    const wantedMedia = await Media.find({ 'album.owned': false })
    //display the wanted albums
    res.render("wantedmedia/index.ejs", { medias: wantedMedia })
})
...
router.put("/:id", async (req, res) => {
    try {
        ...
        console.log("Updated wanted media:", updatedMedia);
        res.redirect(`/wanted/${req.params.id}`);
    } catch (error) {
        console.error(err);
        res.status(500).send("Internal Server Error");
    }
...
router.get("/edit/:id", async (req, res) => {
    try {
        ...
        res.render("wantedmedia/edit.ejs", {media: foundMedia})
    } catch (error) {
        res.status(500).send("Internal Server Error");
    }
})
